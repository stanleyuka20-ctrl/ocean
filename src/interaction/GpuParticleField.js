...
import { PARTICLE_UPDATE_FRAG, DROPLET_VERT, DROPLET_FRAG, BUBBLE_FRAG,
...
export class GpuParticleField {
  ...
  _buildMesh(id) {
    ...
    // motes and bubbles are both lit from under the surface, so neither can use
    // the spray shader's sky integral
    const underLit = isBubble || isMote;
    BJ.Effect.ShadersStore[`${id}VertexShader`] = DROPLET_VERT;
    BJ.Effect.ShadersStore[`${id}FragmentShader`] =
      isMote ? MOTE_FRAG : (isBubble ? BUBBLE_FRAG : DROPLET_FRAG);
    ...
    // --- velocity variant, for the TAA velocity target --------------------
    BJ.Effect.ShadersStore[`${id}VelVertexShader`] = PARTICLE_VEL_VERT;
    ...
    this.velMaterial.backFaceCulling = false;
  ...
  update(dt, ctx) {
    if (!this._built || !this.ew.isReady()) { this._groups.length = 0; return; }
